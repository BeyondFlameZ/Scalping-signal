...
def calc(s):
 ...
 rv=sum(x[1] for x in r);cv=clip(rv/(sum(abs(x[1]) for x in r)+1e-9));old=abs(sum(x[1] for x in q))/max(1,len(q));acc=(abs(rv)/len(r))/(old+1e-9)
 sc=(28 if cv>.45 else -28 if cv<-.45 else 0)+(18 if acc>1.5 and rv>0 else -18 if acc>1.5 and rv<0 else 0)+(14 if abs(cv)>.65 else 0)
 return {"score":sc,"cvd":cv,"accel":acc}
...
def closep(s,why,px):
 ...
 if not p:return
 pnl=(px-p["entry"])*p["qty"]*(1 if p["side"]=="LONG" else -1)-(p["entry"]+px)*p["qty"]*.00055;S["balance"]+=pnl;S["pnl"]+=pnl;S["trades"]+=1;S["wins"]+=pnl>=0;S["losses"]+=pnl<0;ev({"t":time.time(),"event":why,"symbol":s,"side":p["side"],"pnl":round(pnl,2)})
async def trade(x):
 ...
 if z:
  ...
  elif z["score"]<=-TH:S["short_setups"]+=1;openp(s,"SHORT",px,z["score"])
 p=S["positions"].get(s)
 if p:
...
def status():
 ...
 return JSONResponse({"balance":round(S["balance"],2),"equity":round(eq,2),"pnl":round(S["pnl"],2),"ws":S["ws"],"position_count":len(S["positions"]),"max_positions":MAXP,"monitor":MON,"calculations":S["calculations"],"long_setups":S["long_setups"],"short_setups":S["short_setups"],"trades":S["trades"],"wins":S["wins"],"losses":S["losses"],"last_market":S["last_market"],"last_error":S["last_error"],"diagnostics":[{"symbol":k,**v} for k,v in z],"events":list(S["events"])})
@app.get("/",response_class=HTMLResponse)
def home():
 return """<meta name="viewport" content="width=device-width,initial-scale=1"><style>body{font:14px system-ui;background:#111;color:#eee;margin:14px}.box{border:1px solid #333;border-radius:12px;padding:12px;margin:8px 0}table{width:100%;font-size:11px}td,th{padding:4px;text-align:right}td:first-child,th:first-child{text-align:left}pre{font-size:11px;white-space:pre-wrap}</style><h2>⚡ Micro Scalper — DIAGNOSTIC v3</h2><div id="a" class="box">Loading...</div><div class="box"><b>Live scores</b><div style="overflow:auto"><table id="t"></table></div></div><div class="box"><b>Events</b><pre id="e">Loading...</pre></div><script>
async function g(){try{const r=await fetch("/api/status?x="+Date.now());if(!r.ok)throw Error("HTTP "+r.status);const x=await r.json();document.getElementById("a").innerHTML="Balance $"+x.balance+" | Equity $"+x.equity+" | PnL $"+x.pnl+"<br>WS "+x.ws+" | Positions "+x.position_count+"/"+x.max_positions+" | Monitor "+x.monitor+"<br>Calculations "+x.calculations+" | LONG "+x.long_setups+" | SHORT "+x.short_setups+"<br>Trades "+x.trades+" W/L "+x.wins+"/"+x.losses+"<br>Last "+(x.last_market?new Date(x.last_market*1000).toLocaleTimeString():"-")+" | Error "+(x.last_error||"-");document.getElementById("t").innerHTML="<tr><th>Symbol</th><th>Score</th><th>CVD</th><th>Accel</th></tr>"+x.diagnostics.map(z=>"<tr><td>"+z.symbol+"</td><td>"+z.score.toFixed(1)+"</td><td>"+z.cvd.toFixed(2)+"</td><td>"+z.accel.toFixed(2)+"</td></tr>").join("");document.getElementById("e").textContent=x.events.map(z=>new Date(z.t*1000).toLocaleTimeString()+" "+z.event+" "+z.symbol+" "+z.side+" score="+(z.score||"")+" pnl="+(z.pnl??"")).join("\\n")}catch(e){document.getElementById("a").textContent="Dashboard error: "+e.message}}g();setInterval(g,1000)</script>"""
